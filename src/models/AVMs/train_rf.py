...
class RFTrainConfig:
    ...
    min_samples_split_grid: Tuple[int, ...] = (2, 5, 10)
    min_samples_leaf_grid: Tuple[int, ...] = (1, 2, 4)
    max_features_grid: Tuple[str | float, ...] = ("sqrt", 0.5)
...
def tune_rf(
    ...
    param_grid: Dict[str, Any] = {
        "rf__n_estimators": list(cfg.n_estimators_grid),
        "rf__max_depth": list(cfg.max_depth_grid),
        "rf__min_samples_split": list(cfg.min_samples_split_grid),
        "rf__min_samples_leaf": list(cfg.min_samples_leaf_grid),
        "rf__max_features": list(cfg.max_features_grid),
    }
...
def save_rf_artifacts(
    ...
    cv_path = cfg.out_dir / "rf_cv_results.csv"
    ...
    metrics_csv_path = cfg.out_dir / "rf_metrics_train.csv"
    pd.DataFrame([metrics_row]).to_csv(metrics_csv_path, index=False, encoding="utf-8-sig")

    # TXT report
    txt_path = cfg.out_dir / "rf_metrics_train.txt"
    txt = (
        "Random Forest training summary (TRAIN ONLY)\n"
        "==================================================\n"
    ...
    json_path = cfg.out_dir / "rf_summary.json"
    ...
    # Model
    model_path = cfg.out_dir / "rf_best_model.joblib"
...
# Runner
# ============================================================
def train_rf_from_train_for_avms(cfg: RFTrainConfig) -> GridSearchCV:
    ...
    pipeline = build_rf_pipeline(df, cfg.month_col, cfg.district_col, cfg.target_col)
    gs = tune_rf(X, y, cfg, pipeline)  # type: ignore

    print("\n=== Random Forest Training (GridSearchCV) Completed ===")
...
def main() -> None:
    ...
    out_dir = MODEL_DIR / "avms" / "rf_best_model"
    ...
    train_rf_from_train_for_avms(cfg)
